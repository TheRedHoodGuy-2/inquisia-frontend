...
function PdfDropZone({ file, onFile }: { file: File | null; onFile: (f: File | null) => void }) {
  ...
  if (file) {
    return (
      <div className="rounded-2xl border border-[#0066FF] bg-[#0066FF08] p-3 flex items-center gap-3">
        ...
        <p className="text-[12px] text-[#0A0A0F] dark:text-[#F5F5F5] flex-1 truncate" style={{ fontFamily: 'var(--font-body)' }}>{file.name}</p>
        <button onClick={() => onFile(null)} className="p-1 rounded-full hover:bg-[#F2F4F7] dark:hover:bg-[#18181D]"><X size={12} className="text-[#6B7280]" /></button>
      </div>
    )
  }
  ...
  return (
    <div
      className={`rounded-2xl border-2 border-dashed p-5 text-center cursor-pointer transition-all duration-150 ${dragging ? 'border-[#0066FF] bg-[#0066FF08]' : 'border-[#C9D0DA] dark:border-[#222229] hover:border-[#0066FF]'}`}
  ...
  return (
    <AnimatePresence>
      ...
      <motion.div
        ...
        className="fixed inset-x-4 top-1/2 -translate-y-1/2 max-w-lg mx-auto bg-white dark:bg-[#111115] z-50 rounded-2xl shadow-2xl border border-[#C9D0DA] dark:border-[#222229] overflow-hidden"
        ...
        {submitted ? (
          ...
          <>
            <div className="px-6 py-5 space-y-5 max-h-[65vh] overflow-y-auto">
              {/* Project context */}
              <div className="p-3 rounded-2xl bg-[#F2F4F7] dark:bg-[#18181D] border border-[#C9D0DA] dark:border-[#222229]">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {CHANGEABLE_FIELDS.map((field) => {
                    ...
                    return (
                      <div key={field.key} className={`rounded-2xl border transition-all duration-150 overflow-hidden ${isSelected ? 'border-[#0066FF]' : 'border-[#C9D0DA] dark:border-[#222229]'}`}>
                        {/* Toggle row */}
                        <button
                          onClick={() => toggleField(field.key)}
                          className={`w-full flex items-start gap-3 p-3 text-left ${isSelected ? 'bg-[#0066FF08]' : 'hover:bg-[#F2F4F7] dark:hover:bg-[#18181D]'}`}
                        >
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-all ${isSelected ? 'border-[#0066FF] bg-[#0066FF]' : 'border-[#C9D0DA] dark:border-[#222229]'}`}>
                            {isSelected && <Check size={10} weight="bold" className="text-white" />}
                          </div>
                          <div>
                            <p className={`text-[13px] ${isSelected ? 'text-[#0066FF]' : 'text-[#0A0A0F] dark:text-[#F5F5F5]'}`} style={{ fontFamily: 'var(--font-body)', fontWeight: 500 }}>
                              {field.label}
                            </p>
                          ...
                          {isSelected && (
                            ...
                            >
                              <div className="px-3 pb-3 space-y-2">
                                ...
                                {currentVal && (
                                  <div className="rounded-xl bg-[#F2F4F7] dark:bg-[#18181D] px-3 py-2">
                                ...
                                {/* New value input */}
                                <div className="rounded-xl border border-[#C9D0DA] dark:border-[#222229] focus-within:border-[#0066FF] transition-colors bg-white dark:bg-[#111115]">
                                  ...
                                  {field.type === 'textarea' ? (
                                    <textarea
                                      ...
                                      className="w-full bg-transparent outline-none px-3 pb-2 pt-1 text-[12px] text-[#0A0A0F] dark:text-[#F5F5F5] placeholder-[#6B7280] resize-none"
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => setProposed(field.key, e.target.value)}
                                      placeholder={field.placeholder}
                                      className="w-full bg-transparent outline-none px-3 pb-2 pt-1 text-[12px] text-[#0A0A0F] dark:text-[#F5F5F5] placeholder-[#6B7280]"
                                      style={{ fontFamily: 'var(--font-body)' }}
                                    />
                                  )}
                                </div>
              ...
              <div>
                ...
                <div className="relative rounded-2xl border border-[#C9D0DA] dark:border-[#222229] bg-white dark:bg-[#111115] focus-within:border-[#0066FF] transition-colors">
                  <textarea
                    value={reason}
                    onChange={(e) => setReason(e.target.value)}
                    rows={3}
                    placeholder="Explain clearly what needs to change and why (min. 20 characters)..."
                    className="w-full bg-transparent outline-none px-4 py-3 text-[13px] text-[#0A0A0F] dark:text-[#F5F5F5] placeholder-[#6B7280] resize-none"
                    style={{ fontFamily: 'var(--font-body)' }}
                  />
