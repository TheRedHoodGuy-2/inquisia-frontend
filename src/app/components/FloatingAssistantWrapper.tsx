...
import { Robot, X, PaperPlaneTilt, ChatCircle, ArrowUpRight } from 'phosphor-react'
  ...
  a({ href, children }) {
    ...
    if (isProject) {
      return (
        <Link
          to={href ?? '/'}
          className="group flex items-center justify-between gap-2 my-1.5 px-3 py-2 rounded-lg border border-[#E4E7EC] dark:border-[#222229] bg-white dark:bg-[#111115] hover:border-[#0066FF] hover:bg-[rgba(0,102,255,0.04)] transition-all duration-200 no-underline"
          style={{ fontFamily: 'var(--font-body)', display: 'flex' }}
        >
          <span className="flex items-center gap-2 min-w-0">
            <span className="w-5 h-5 rounded-md bg-[rgba(0,102,255,0.1)] flex items-center justify-center flex-shrink-0">
              <ArrowUpRight size={11} weight="bold" className="text-[#0066FF]" />
            </span>
            <span className="text-[12px] font-medium text-[#0A0A0F] dark:text-[#F5F5F5] group-hover:text-[#0066FF] truncate transition-colors">
              {children}
            </span>
          </span>
          <span className="text-[10px] text-[#9CA3AF] flex-shrink-0 group-hover:text-[#0066FF] transition-colors">View →</span>
        </Link>
      )
    }
    return (
      <a href={href} target="_blank" rel="noopener noreferrer" className="text-[#0066FF] underline underline-offset-2 hover:opacity-80 transition-opacity">
        {children}
      </a>
    )
  },
...
function MessageBubble({ msg, isLatestAI }: { msg: ChatMessage; isLatestAI: boolean }) {
  ...
  return (
    <div className={`flex items-end gap-2 mb-3 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div
        className={`max-w-[85%] min-w-0 overflow-hidden px-3.5 py-2.5 text-[13px] leading-relaxed shadow-sm ${isUser
          ? 'bg-[#0066FF] text-white rounded-2xl rounded-br-none'
          : 'bg-[#F2F4F7] dark:bg-[#18181D] text-[#0A0A0F] dark:text-[#F5F5F5] rounded-2xl rounded-bl-none border border-[#E4E7EC] dark:border-[#222229]'
          }`}
      ...
      >
        {isUser ? (
          ...
          <div className="prose prose-xs dark:prose-invert max-w-none break-words prose-p:leading-relaxed prose-a:no-underline">
            {displayed ? (
...
export function FloatingAssistantWrapper() {
  ...
  const [clipped, setClipped] = useState<'left' | 'right' | null>(null)
  ...
  const onDragMove = useCallback((e: React.PointerEvent) => {
    ...
    const x = Math.max(-BTN_SIZE / 2, Math.min(W - BTN_SIZE / 2, drag.current.originX + dx))
    const y = Math.max(72, Math.min(H - BTN_SIZE - 72, drag.current.originY + dy))

    setClipped(x < 12 ? 'left' : x > W - BTN_SIZE - 12 ? 'right' : null)
    setPos({ x, y })
  }, [])
  ...
  const onDragEnd = useCallback((e: React.PointerEvent) => {
    ...
    // Snap to clipped edge position
    const W = window.innerWidth
    setPos(p => {
      if (!p) return p
      if (p.x < 12) return { ...p, x: -BTN_SIZE / 2 + 4 }
      if (p.x > W - BTN_SIZE - 12) return { ...p, x: W - BTN_SIZE / 2 - 4 }
      return p
    })
  }, [])
  ...
  const send = useCallback(async () => {
    ...
    const res = await aiApi.assistant(trimmed, history, getContext(), settings?.model, settings?.response_style)
  ...
  return (
    <>
      {/* Floating button — draggable on mobile, clips to edge with reduced opacity */}
      <AnimatePresence>
        {!open && (
          <motion.div
            initial={{ scale: 0, opacity: 0 }}
            animate={{ scale: 1, opacity: clipped ? 0.35 : 1 }}
            ...
            className="rounded-full bg-[#0066FF] text-white shadow-[0_4px_24px_rgba(0,102,255,0.4)] flex items-center justify-center pulse-ring select-none"
            role="button"
            tabIndex={0}
            aria-label="Open Elara AI assistant"
            onKeyDown={(e) => e.key === 'Enter' && setOpen(true)}
          >
            <ElaraLogo variant="blue" className="w-6 h-6 pointer-events-none" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-[5.5rem] sm:bottom-6 right-4 sm:right-6 z-[70] w-[calc(100vw-32px)] sm:w-[380px] h-[calc(100dvh-7rem-64px)] sm:h-[520px] max-h-[80dvh] sm:max-h-[80vh] rounded-2xl bg-white dark:bg-[#111115] shadow-[0_16px_64px_rgba(0,0,0,0.2)] border border-[#E4E7EC] dark:border-[#222229] flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex-shrink-0">
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#E4E7EC] dark:border-[#222229]">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-[14px] font-semibold text-[#0A0A0F] dark:text-[#F5F5F5] leading-none" style={{ fontFamily: 'var(--font-display)' }}>
                      Elara
                    </p>
                ...
                <button
                  ...
                  className="p-1.5 rounded-full text-[#9CA3AF] hover:text-[#0A0A0F] dark:hover:text-[#F5F5F5] hover:bg-[#F2F4F7] dark:hover:bg-[#18181D] transition-colors"
                >
                  <X size={16} />
                </button>
              </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 justify-center">
                    {suggestions.map((suggestion) => (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-full border border-[#E4E7EC] dark:border-[#222229] text-[12px] text-[#5C6070] dark:text-[#8B8FA8] hover:border-[#0066FF] hover:text-[#0066FF] transition-colors"
                      >
                        {suggestion}
                      </button>
                    ))}
            ...
            {/* Input */}
            <div className="flex-shrink-0 px-3 pb-3 border-t border-[#E4E7EC] dark:border-[#222229] pt-3">
              {!user ? (
                ...
                <div className="flex items-end gap-2 bg-[#F2F4F7] dark:bg-[#18181D] rounded-2xl px-4 py-2.5 border border-[#E4E7EC] dark:border-[#222229] focus-within:border-[#0066FF] focus-within:shadow-[0_0_0_3px_rgba(0,102,255,0.15)] transition-all">
                  <textarea
                    ref={inputRef as any}
                    value={input}
                    onChange={(e) => {
                    ...
                    rows={1}
                    className="flex-1 bg-transparent outline-none text-[13px] text-[#0A0A0F] dark:text-[#F5F5F5] placeholder-[#9CA3AF] resize-none overflow-y-auto max-h-[120px]"
