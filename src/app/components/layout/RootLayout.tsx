...
const SELF_LAYOUT_PATHS = ['/elara', '/dashboard', '/admin']
...
function Layout() {
  ...
  const selfLayout = SELF_LAYOUT_PATHS.some((p) => location.pathname.startsWith(p))
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#FFFFFF] dark:bg-[#080808]">
      ...
      {/* Main content — padded top to clear fixed navbar; bottom padding for mobile dock on non-self-layout pages */}
      <main className={`flex-1 pt-16 ${selfLayout ? '' : 'pb-[65px] md:pb-0'}`}>
