          ...
          <React.Fragment key={step}>
            <div className="flex flex-col items-center">
              <motion.div
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center text-[13px] transition-all duration-200 ${completed ? 'bg-[#0066FF] text-white' : active ? 'bg-[#0066FF] text-white' : 'bg-[#F2F4F7] dark:bg-[#18181D] text-[#9CA3AF]'
                  }`}
            ...
            {i < totalSteps - 1 && (
              <div className={`h-px flex-1 mx-2 mb-4 transition-colors duration-200 ${step < currentStep ? 'bg-[#0066FF]' : 'bg-[#E4E7EC] dark:bg-[#222229]'}`} />
            )}
          </React.Fragment>
        )
      })}
...
function DropZone({ file, onFile, accept = 'application/pdf', label = 'Drop your PDF here or click to browse' }: {
    ...
    return (
      <div className="rounded-2xl border border-[#0066FF] bg-[#0066FF08] p-5 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[14px] font-medium text-[#0A0A0F] dark:text-[#F5F5F5] truncate" style={{ fontFamily: 'var(--font-body)' }}>{file.name}</p>
          <p className="text-[12px] text-[#9CA3AF]" style={{ fontFamily: 'var(--font-body)' }}>{(file.size / 1024 / 1024).toFixed(1)} MB</p>
        </div>
        <button onClick={() => onFile(null)} className="p-1.5 rounded-full hover:bg-[#F2F4F7] dark:hover:bg-[#18181D]"><X size={14} className="text-[#9CA3AF]" /></button>
      </div>
    )
  }
  ...
  return (
    <div
      className={`rounded-2xl border-2 border-dashed p-10 text-center cursor-pointer transition-all duration-150 ${dragging ? 'border-[#0066FF] bg-[#0066FF08]' : 'border-[#E4E7EC] dark:border-[#222229] hover:border-[#0066FF] hover:bg-[#0066FF08]'
        }`}
    ...
    >
      ...
      <p className="text-[14px] text-[#5C6070] dark:text-[#8B8FA8]" style={{ fontFamily: 'var(--font-body)' }}>{label}</p>
...
function StepCard({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] p-6"
...
function TagInput({ tags, onAdd, onRemove }: { tags: string[]; onAdd: (t: string) => void; onRemove: (t: string) => void }) {
  ...
  return (
    <div className="rounded-2xl border border-[#E4E7EC] dark:border-[#222229] p-3 bg-white dark:bg-[#111115] focus-within:border-[#0066FF]">
      ...
      <input
        ...
        className="w-full bg-transparent outline-none text-[13px] text-[#0A0A0F] dark:text-[#F5F5F5] placeholder-[#9CA3AF]"
...
function FloatingLabel({ id, label, value, onChange, type = 'text' }: { id: string; label: string; value: string; onChange: (v: string) => void; type?: string }) {
  ...
  return (
    <div className={`relative rounded-full border transition-all duration-150 bg-white dark:bg-[#111115] ${f ? 'border-[#0066FF] shadow-[0_0_0_3px_rgba(0,102,255,0.15)]' : 'border-[#E4E7EC] dark:border-[#222229]'}`}>
...
export function UploadPage() {
  ...
  const [validationResult, setValidationResult] = useState<{ success: boolean; message: string; category?: string; suggested_abstract?: string; suggested_prompt?: string; pdfText?: string; plagiarismData?: { score: number, similarProjectId?: string, similarityReason?: string } } | null>(null)
    ...
    if (res.success) {
      if (res.data.valid === false) {
        setValidationResult({
          ...
          suggested_abstract: res.data.suggested_abstract,
  ...
  if (isLoading) return <div className="flex items-center justify-center min-h-[60vh]"><div className="w-8 h-8 rounded-full border-2 border-[#E4E7EC] border-t-[#0066FF] animate-spin" /></div>

  return (
    <DashboardLayout activeSection="upload">
      <div className="max-w-[600px] mx-auto">
        <div className="mb-8">
          <h1 className="text-[#0A0A0F] dark:text-[#F5F5F5] mb-2" style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '24px', letterSpacing: '-0.02em' }}>
            Submit a Project
          </h1>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div key="step1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.22, ease: 'easeOut' }}>
              <StepCard>
                <h2 className="mb-5 text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '18px' }}>Upload Files</h2>
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.22, ease: 'easeOut' }}>
              <StepCard>
                <h2 className="mb-5 text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '18px' }}>Project Details</h2>
                <div className="space-y-4">
                  <FloatingLabel id="title" label="Project title" value={title} onChange={setTitle} />
                  <div className="relative rounded-2xl border border-[#E4E7EC] dark:border-[#222229] bg-white dark:bg-[#111115] p-4">
                    <textarea value={abstract} onChange={(e) => setAbstract(e.target.value)} rows={5} placeholder="Project abstract (min. 50 characters)"
                      className="w-full bg-transparent outline-none text-[14px] text-[#0A0A0F] dark:text-[#F5F5F5] placeholder-[#9CA3AF] resize-none"
                      style={{ fontFamily: 'var(--font-body)' }} />
                          ...
                          <div className="ml-8 mt-1 flex flex-col gap-2">
                            <div className="flex flex-wrap items-center gap-2">
                              {validationResult.suggested_prompt && (
                                <button
                                  onClick={() => {
                                    const event = new CustomEvent('open-elara', { detail: { prompt: validationResult.suggested_prompt, context: validationResult.pdfText || extractedPdfText } })
                                    window.dispatchEvent(event)
                                  }}
                                  className="px-4 py-2 rounded-full border border-red-200 bg-white hover:bg-red-50 text-red-600 text-[12px] font-medium transition-colors">
                                  ✨ Ask Elara for Help
                                </button>
                              )}
                              <button
                                onClick={() => void handleValidate()}
                                disabled={!title || abstract.length < 50 || validating}
                                className="px-4 py-2 rounded-full border border-[#E4E7EC] bg-white hover:bg-[#F8F9FB] text-[#5C6070] text-[12px] font-medium transition-colors disabled:opacity-40">
                                Re-validate
                              </button>
                            </div>
                            <p className="text-[11px] text-[#9AA0AD]" style={{ fontFamily: 'var(--font-body)' }}>
                              Apply the suggestion above or edit your abstract, then click Re-validate.
                            </p>
                          </div>
                        )}
                      </motion.div>
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.22, ease: 'easeOut' }}>
              <StepCard>
                <h2 className="mb-5 text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '18px' }}>Team Members</h2>
                {/* Uploading student */}
                <div className="rounded-2xl border border-[#0066FF] bg-[#0066FF08] p-4 mb-4 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[13px] font-semibold text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)' }}>{user?.full_name ?? 'You'}</p>
                ...
                <div className="flex gap-2 mb-4">
                  <div className="flex-1 flex items-center gap-2 rounded-full border border-[#E4E7EC] dark:border-[#222229] bg-white dark:bg-[#111115] px-4 py-2.5">
                    ...
                    <input value={searchMatric} onChange={(e) => setSearchMatric(e.target.value)} placeholder="Search by matric number"
                      className="flex-1 bg-transparent outline-none text-[13px] placeholder-[#9CA3AF] text-[#0A0A0F] dark:text-[#F5F5F5]"
                      style={{ fontFamily: 'var(--font-body)' }} />
                  </div>
                ...
                {searchResult && (
                  <div className="rounded-2xl border border-[#E4E7EC] dark:border-[#222229] p-3 mb-4 flex items-center justify-between">
                    <div>
                      <p className="text-[13px] font-medium text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)' }}>{searchResult.name}</p>
                ...
                {teammates.map((t, i) => (
                  <div key={t.id} className="rounded-2xl border border-[#E4E7EC] dark:border-[#222229] p-4 mb-3 flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#F2F4F7] dark:bg-[#18181D] flex items-center justify-center text-[#9CA3AF] text-[11px]" style={{ fontFamily: 'var(--font-display)', fontWeight: 700 }}>
                      {t.name[0]}
                    </div>
                    <div className="flex-1">
                      <p className="text-[13px] font-medium text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)' }}>{t.name}</p>
                    ...
                    <button onClick={() => setTeammates((prev) => prev.filter((_, mi) => mi !== i))} className="p-1.5 rounded-full hover:bg-[#F2F4F7] dark:hover:bg-[#18181D]">
                      <X size={13} className="text-[#9CA3AF]" />
                    </button>
                  </div>
                ))}
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.22, ease: 'easeOut' }}>
              <StepCard>
                <h2 className="mb-5 text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '18px' }}>Select Supervisor</h2>
                <div className="space-y-3">
                  {supervisors.map((s) => (
                    <label key={s.id} className={`flex items-start gap-3 p-4 rounded-2xl border cursor-pointer transition-all duration-150 ${selectedSupervisor === s.id ? 'border-[#0066FF] bg-[#0066FF08]' : 'border-[#E4E7EC] dark:border-[#222229] hover:border-[#0066FF]/40'
                      }`}>
                      ...
                      <div>
                        <p className="text-[14px] font-semibold text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)' }}>{s.full_name}</p>
          ...
          {step === 5 && (
            <motion.div key="step5" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.22, ease: 'easeOut' }}>
              <StepCard>
                <h2 className="mb-5 text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '18px' }}>Review & Submit</h2>
                <div className="space-y-4">
                  {[
                    ...
                    <div key={label} className="flex gap-3 rounded-2xl bg-[#F7F8FA] dark:bg-[#18181D] p-4">
                      ...
                      <p className="text-[13px] text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-body)' }}>{value}</p>
                    </div>
                  ))}
                </div>
              </StepCard>
        ...
        <div className="flex items-center justify-between mt-6">
          <button
            ...
            disabled={step === 1}
            className="px-5 py-2.5 rounded-full text-[13px] border border-[#E4E7EC] dark:border-[#222229] text-[#5C6070] disabled:opacity-40 hover:border-[#0066FF] hover:text-[#0066FF] transition-colors"
            style={{ fontFamily: 'var(--font-body)', fontWeight: 500 }}
          >
            Back
          </button>
        ...
        {showSimilarityWarning && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }}
              className="relative w-full max-w-[500px] bg-white dark:bg-[#111115] rounded-3xl shadow-xl overflow-hidden shadow-black/10 border border-[#E4E7EC] dark:border-[#222229]">
              {/* Header */}
              <div className="flex items-center justify-between p-5 border-b border-[#E4E7EC] dark:border-[#222229]">
                <h3 className="text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '18px' }}>Similarity Warning</h3>
                <button onClick={() => setShowSimilarityWarning(false)} className="p-2 -mr-2 text-[#9CA3AF] hover:text-[#0A0A0F] dark:hover:text-[#F5F5F5] transition-colors"><X size={20} /></button>
              </div>
              {/* Body */}
              <div className="p-6">
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div>
                    <h4 className="text-[15px] font-semibold text-[#0A0A0F] dark:text-[#F5F5F5] mb-1" style={{ fontFamily: 'var(--font-display)' }}>High Similarity Detected ({validationResult?.plagiarismData?.score}%)</h4>
                    <p className="text-[13px] text-[#5C6070] leading-relaxed mb-3" style={{ fontFamily: 'var(--font-body)' }}>
                ...
                <div className="flex justify-end gap-3">
                  <button onClick={() => setShowSimilarityWarning(false)} className="px-5 py-2.5 rounded-full text-[13px] font-medium text-[#5C6070] bg-[#F2F4F7] dark:bg-[#18181D] hover:bg-[#E4E7EC] dark:hover:bg-[#2A2A2A] transition-colors">Go Back & Edit</button>
