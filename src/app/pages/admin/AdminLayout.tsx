...
import { ChartBar, Users, FolderOpen, GraduationCap, Buildings, Robot, Warning, Megaphone, Bug } from 'phosphor-react'
...
const NAV_ITEMS = [
  ...
  { icon: <Bug size={18} />, label: 'Feedback', href: '/admin/feedback' },
]
...
export function AdminLayout() {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-[calc(100vh-64px)]">
      {/* Sidebar */}
      <aside className="w-56 flex-shrink-0 hidden md:flex flex-col border-r border-[#E4E7EC] dark:border-[#222229] bg-white dark:bg-[#111115]">
              ...
              return (
                <Link key={item.href} to={item.href}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-full text-[13px] transition-colors duration-150 ${isActive
                    ...
                    : 'text-[#5C6070] dark:text-[#8B8FA8] hover:bg-[#F2F4F7] dark:hover:bg-[#18181D] hover:text-[#0A0A0F] dark:hover:text-[#F5F5F5]'
                    }`}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Mobile Nav */}
        <nav className="md:hidden flex overflow-x-auto gap-2 border-b border-[#E4E7EC] dark:border-[#222229] bg-white dark:bg-[#111115] p-3 scrollbar-none">
          {NAV_ITEMS.map((item) => {
            const isActive = item.href === '/admin'
              ? location.pathname === '/admin'
              : location.pathname.startsWith(item.href)
