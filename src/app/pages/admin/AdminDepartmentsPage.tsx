    ...
    <div>
      ...
      <div className="flex gap-2 mb-6">
        <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)}
          ...
          className="flex-1 bg-white dark:bg-[#111115] rounded-full border border-[#E4E7EC] dark:border-[#222229] px-4 py-2.5 text-[14px] text-[#0A0A0F] dark:text-[#F5F5F5] placeholder-[#9CA3AF] outline-none focus:border-[#0066FF] transition-colors"
      ...
      <div className="rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] overflow-hidden"
        ...
        {loading ? (
          <div className="py-10 text-center"><div className="w-6 h-6 rounded-full border-2 border-[#E4E7EC] border-t-[#0066FF] animate-spin mx-auto" /></div>
        ) : departments.length === 0 ? (
          <div className="py-10 text-center"><p className="text-[14px] text-[#9CA3AF]">No departments yet.</p></div>
        ) : departments.map((d, i) => (
          <div key={d.id} className={`flex items-center gap-4 px-5 py-4 group hover:bg-[#0066FF08] transition-colors ${i > 0 ? 'border-t border-[#E4E7EC] dark:border-[#222229]' : ''}`}>
            {editId === d.id ? (
              <>
                <input value={editName} onChange={(e) => setEditName(e.target.value)}
                  className="flex-1 bg-transparent outline-none text-[14px] text-[#0A0A0F] dark:text-[#F5F5F5] border-b border-[#0066FF]"
              ...
              <>
                <span className="flex-1 text-[14px] text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-body)' }}>{d.name}</span>
