...
function StatCard({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: number | string; color: string }) {
  return (
    <div className="rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] p-5"
      ...
      <p className="text-[28px] font-bold text-[#0A0A0F] dark:text-[#F5F5F5]"
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={s.label} className="rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] p-4"
            style={{ boxShadow: 'var(--shadow-card)' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  return (
                    <div key={p.id} className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="text-[13px] font-medium text-[#0A0A0F] dark:text-[#F5F5F5] truncate" style={{ fontFamily: 'var(--font-display)' }}>{p.title}</p>
            ...
            <div className="space-y-2">
              {[
                ...
                <Link key={link.href} to={link.href}
                  className="block px-4 py-2.5 rounded-full text-[13px] text-[#5C6070] dark:text-[#8B8FA8] hover:bg-[#0066FF1A] hover:text-[#0066FF] transition-colors"
        ...
        <div className="rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] p-5"
          style={{ boxShadow: 'var(--shadow-card)' }}>
          <h2 className="text-[15px] text-[#0A0A0F] dark:text-[#F5F5F5] mb-4" style={{ fontFamily: 'var(--font-display)', fontWeight: 600 }}>Top Downloads</h2>
          <div className="space-y-3">
            {topProjects.map((p, i) => (
              <Link key={p.id} to={`/projects/${p.id}`} className="flex items-center gap-3 group">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] text-[#0A0A0F] dark:text-[#F5F5F5] truncate group-hover:text-[#0066FF] transition-colors" style={{ fontFamily: 'var(--font-body)' }}>
                    {p.title}
                  </p>
                </div>
