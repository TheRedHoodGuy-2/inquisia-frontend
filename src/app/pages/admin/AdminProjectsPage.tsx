  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="flex-1 flex items-center gap-2 bg-white dark:bg-[#111115] rounded-full border border-[#E4E7EC] dark:border-[#222229] px-4 py-2.5">
          ...
          <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search projects..."
            className="flex-1 bg-transparent outline-none text-[14px] text-[#0A0A0F] dark:text-[#F5F5F5] placeholder-[#9CA3AF]"
        ...
        <div className="flex gap-1 flex-wrap">
          {['', 'pending', 'approved', 'changes_requested', 'rejected'].map((s) => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`px-3 py-2 rounded-full text-[12px] transition-colors ${statusFilter === s ? 'bg-[#0066FF] text-white' : 'bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] text-[#5C6070] hover:border-[#0066FF] hover:text-[#0066FF]'}`}
      ...
      <div className="rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] overflow-hidden"
        ...
        {loading ? (
          ...
          filtered.map((p, i) => {
            ...
            return (
              <div key={p.id} className={`flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 px-5 py-4 hover:bg-[#0066FF08] transition-colors group ${i > 0 ? 'border-t border-[#E4E7EC] dark:border-[#222229]' : ''}`}>
                <div className="flex-1 min-w-0">
                  <Link to={`/projects/${p.id}`} className="text-[13px] font-semibold text-[#0A0A0F] dark:text-[#F5F5F5] hover:text-[#0066FF] transition-colors truncate block"
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {p.plagiarism_score !== null && (
                    <span className={`inline-flex items-center gap-1.5 font-medium ${p.plagiarism_score > 30 ? 'text-[#DC2626] dark:text-red-400' : 'text-[#9CA3AF] dark:text-[#5C6070]'}`}>
                      {p.plagiarism_score}%
