        ...
        <div className="flex flex-wrap gap-1">
          {['', 'student', 'supervisor', 'admin'].map((r) => (
            <button key={r} onClick={() => setRoleFilter(r)}
              className={`px-3 py-2 rounded-full text-[12px] transition-colors ${roleFilter === r ? 'bg-[#0066FF] text-white' : 'bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] text-[#5C6070] hover:border-[#0066FF] hover:text-[#0066FF]'}`}
        ...
        {loading ? (
          ...
          <div>
            {users.map((user, i) => {
              ...
              return (
                <div key={user.id}
                  className={`flex flex-col sm:flex-row sm:items-center gap-3 px-5 py-4 hover:bg-[#0066FF08] transition-colors group ${i > 0 ? 'border-t border-[#E4E7EC] dark:border-[#222229]' : ''}`}>
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-semibold text-[#0A0A0F] dark:text-[#F5F5F5] truncate" style={{ fontFamily: 'var(--font-display)' }}>
