        ...
        return (
          <div key={v.id} className="flex gap-3">
            <div className="flex flex-col items-center">
              <div className={`w-3 h-3 rounded-full flex-shrink-0 mt-1 ${isCurrent ? 'bg-[#0066FF]' : 'bg-[#E4E7EC] dark:bg-[#222229]'}`} />
              {i < versions.length - 1 && <div className="w-px flex-1 bg-[#E4E7EC] dark:bg-[#222229] my-1" style={{ minHeight: 20 }} />}
            </div>
            <div className="flex-1 pb-2">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[12px] font-semibold text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)' }}>
              ...
              {v.supervisor_feedback && (
                <p className="text-[12px] text-[#5C6070] dark:text-[#8B8FA8] mt-1 leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>
                  {v.supervisor_feedback}
                </p>
              )}
            </div>
...
function StudentProjectCard({ project: initialProject, onDelete }: { project: Project, onDelete?: () => void }) {
  ...
  return (
    <div
      className="rounded-2xl bg-white dark:bg-[#111115] overflow-hidden border border-[#E4E7EC] dark:border-[#222229] border-l-4 transition-colors"
    ...
    >
      <button onClick={() => void handleExpand()} className="w-full text-left p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <h3 className="mb-1 line-clamp-1 text-[#0A0A0F] dark:text-[#F5F5F5]"
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div className="px-5 pb-5 border-t border-[#E4E7EC] dark:border-[#222229] pt-4">
              ...
              {loadingVersions ? (
                <div className="text-center py-4">
                  <div className="w-5 h-5 rounded-full border-2 border-[#E4E7EC] border-t-[#0066FF] animate-spin mx-auto" />
                </div>
              ) : versions.length > 0 ? (
                <div className="mb-4">
                ...
                {project.status === 'approved' && (
                  <>
                    <button onClick={() => setChangeModal(true)}
                      className="px-4 py-2 rounded-full text-[13px] border border-[#E4E7EC] dark:border-[#222229] text-[#5C6070] hover:border-[#0066FF] hover:text-[#0066FF] transition-colors"
                      style={{ fontFamily: 'var(--font-body)', fontWeight: 500 }}>
                      Request Changes
                    </button>
...
function QuickActionCard({ icon, label, desc, href, color }: {
  ...
  return (
    <Link to={href}
      className="flex items-start gap-4 p-4 rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] hover:border-[#0066FF] hover:shadow-[0_4px_16px_rgba(0,102,255,0.08)] transition-all duration-150 group"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[14px] text-[#0A0A0F] dark:text-[#F5F5F5] group-hover:text-[#0066FF] transition-colors" style={{ fontFamily: 'var(--font-display)', fontWeight: 600 }}>{label}</p>
      ...
      {activeSection === 'overview' && (
        <div key="overview">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
            {stats.map((stat) => (
              <div key={stat.label}
                className="flex flex-col gap-1 p-4 rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229]"
                style={{ boxShadow: 'var(--shadow-card)' }}>
                <span className="text-[24px] text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 700, lineHeight: 1.1 }}>{stat.value}</span>
          ...
          {/* Quick actions */}
          <h2 className="text-[15px] text-[#0A0A0F] dark:text-[#F5F5F5] mb-4" style={{ fontFamily: 'var(--font-display)', fontWeight: 600 }}>Quick Actions</h2>
          ...
          {projects.length > 0 && (
            <>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-[15px] text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 600 }}>Recent Projects</h2>
      ...
      {activeSection === 'projects' && (
        <div key="projects">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '24px', letterSpacing: '-0.02em' }}>
                My Projects
              </h1>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 rounded-2xl border border-dashed border-[#E4E7EC] dark:border-[#222229]">
              <FolderOpen size={48} weight="thin" className="text-[#9CA3AF] mx-auto mb-4" />
              <h3 className="text-[16px] text-[#0A0A0F] dark:text-[#F5F5F5] mb-2" style={{ fontFamily: 'var(--font-display)', fontWeight: 600 }}>No projects yet</h3>
...
function SupervisorProjectRow({ project }: { project: Project }) {
  ...
  return (
    <div
      className="rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] overflow-hidden transition-all duration-150"
      style={{ boxShadow: 'var(--shadow-card)' }}
    >
      <Link to={`/projects/${project.id}`}
        className="block p-5 hover:bg-[#F2F4F7] dark:hover:bg-[#18181D] transition-colors"
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <h3 className="mb-1 line-clamp-1 text-[#0A0A0F] dark:text-[#F5F5F5]"
      ...
      {hasVersions && (
        <div className="border-t border-[#E4E7EC] dark:border-[#222229] px-5 py-3">
          {!comparison ? (
            <button
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setComparison(null)} className="text-[11px] text-[#9CA3AF] hover:text-[#5C6070]" style={{ fontFamily: 'var(--font-body)' }}>
                  Close
                </button>
              </div>
              <p className="text-[13px] text-[#5C6070] dark:text-[#8B8FA8] leading-relaxed whitespace-pre-wrap" style={{ fontFamily: 'var(--font-body)' }}>
                {comparison}
              </p>
            </div>
          )}
...
function StudentMiniCard({ author, projectCount }: { author: ProjectAuthor; projectCount: number }) {
  ...
  return (
    <Link to={`/profile/${author.id}`}
      className="flex items-center gap-4 p-4 rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229] hover:border-[#0066FF] transition-all duration-150 group"
      style={{ boxShadow: 'var(--shadow-card)' }}
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[14px] text-[#0A0A0F] dark:text-[#F5F5F5] group-hover:text-[#0066FF] transition-colors truncate"
  ...
  if (user && !user.is_verified) {
    return (
      <div className="text-center py-24">
        ...
        <h2 className="text-[22px] mb-3 text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 700 }}>
      ...
      {activeSection === 'overview' && (
        <motion.div key="overview" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
            {statCards.map((s) => (
              <div key={s.label}
                className="flex flex-col gap-1 p-4 rounded-2xl bg-white dark:bg-[#111115] border border-[#E4E7EC] dark:border-[#222229]"
                style={{ boxShadow: 'var(--shadow-card)' }}>
                <span className="text-[24px] text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 700, lineHeight: 1.1 }}>{s.value}</span>
          ...
          {pending.length > 0 && (
            <>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-[15px] text-[#0A0A0F] dark:text-[#F5F5F5]" style={{ fontFamily: 'var(--font-display)', fontWeight: 600 }}>Needs Review</h2>
      ...
      {(['pending', 'changes', 'approved', 'rejected'] as const).map((key) => (
        activeSection === key && (
          <motion.div key={key} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
            <div className="mb-6">
              <h1 className="text-[#0A0A0F] dark:text-[#F5F5F5]"
            ...
            {loading ? (
              ...
              <div className="text-center py-20 rounded-2xl border border-dashed border-[#E4E7EC] dark:border-[#222229]">
