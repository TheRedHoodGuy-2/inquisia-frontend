...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Abstract */}
      <p className="text-[13px] text-[#5C6370] dark:text-[#8B8FA8] mb-4 line-clamp-3 flex-1" style={{ fontFamily: 'var(--font-body)', lineHeight: 1.6 }}>
        {project.abstract}
      </p>
  ...
  const fetchProjects = useCallback(async (ignore: boolean) => {
    ...
    if (!ignore) {
      if (res.success) {
        ...
        setSemanticActive(res.data.semantic_search ?? false)
      } else {
        setProjects([])
        setTotal(0)
        setTotalPages(1)
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
              ...
              <div className="flex-1 flex items-center gap-2 bg-white dark:bg-[#101010] rounded-full border border-[#E5E7EB] dark:border-[#1C1C1C] px-4 py-2.5">
                ...
                <input
                  ...
                  placeholder="Search by topic, concept, keyword..."
                  className="flex-1 bg-transparent outline-none text-[14px] text-[#0A0A0A] dark:text-[#F5F5F5] placeholder-[#9CA3AF]"
                  style={{ fontFamily: 'var(--font-body)' }}
                />
                <AnimatePresence>
                  {query && semanticActive && (
                    <motion.span
                      initial={{ opacity: 0, scale: 0.8 }}
                      animate={{ opacity: 1, scale: 1 }}
                      exit={{ opacity: 0, scale: 0.8 }}
                      className="flex-shrink-0 px-2 py-0.5 rounded-full text-[10px] font-medium bg-[rgba(0,102,255,0.1)] text-[#0066FF]"
                      style={{ fontFamily: 'var(--font-body)' }}
                    >
                      Semantic
                    </motion.span>
                ...
                {query && (
                  <button onClick={() => { setQuery(''); setPage(1); setSemanticActive(false) }}>
