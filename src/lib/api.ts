...
async function apiFetch<T>(
  ...
  try {
    ...
    // Shape: { success, reply, modelUsed? } — AI assistant/chat endpoints
    if ('reply' in json && !('data' in json)) {
      const d: Record<string, unknown> = { reply: json.reply }
      if ('modelUsed' in json) d.modelUsed = json.modelUsed
      if ('conversation_title' in json) d.conversation_title = json.conversation_title
      return { success: true, data: d as T }
    }
...
export const authApi = {
  ...
  forgotPassword: (email: string) =>
    apiFetch<null>('/api/auth/forgot-password', {
      method: 'POST',
...
export const aiApi = {
  assistant: (
    ...
    pageContext: { path: string; role?: string; projectId?: string; pdfText?: string } = { path: '/' },
    preferredModel?: string,
    responseStyle?: string,
  ) =>
    apiFetch<{ reply: string }>('/api/ai/assistant', {
      method: 'POST',
      body: JSON.stringify({ message, history, pageContext, preferredModel, responseStyle }),
    }),
  elara: (
    message: string,
    history?: { role: string; content: string }[],
    ...
    apiFetch<{ reply: string; modelUsed?: string }>('/api/ai/elara', {
      method: 'POST',
      body: JSON.stringify({ message, history, conversationId, preferredModel, responseStyle }),
    }),
...
export const notificationsApi = {
  ...
  markRead: (id: string) =>
    apiFetch<null>(`/api/notifications/${id}`, { method: 'PATCH' }),
  deleteAll: () =>
...
export const elaraSettingsApi = {
  ...
  update: (data: Partial<ElaraSettings>) =>
    apiFetch<ElaraSettings>('/api/ai/settings', {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
}
...
export const conversationsApi = {
  ...
  create: (title?: string) =>
    apiFetch<ElaraConversation>('/api/ai/conversations', {
      method: 'POST',
      body: JSON.stringify({ title }),
    }),
  get: (id: string) => apiFetch<ElaraConversation & { messages: ElaraMessage[] }>(`/api/ai/conversations/${id}`),
  delete: (id: string) => apiFetch<null>(`/api/ai/conversations/${id}`, { method: 'DELETE' }),
  rename: (id: string, title: string) =>
    apiFetch<ElaraConversation>(`/api/ai/conversations/${id}`, {
      method: 'PATCH',
      body: JSON.stringify({ title }),
    }),
  addMessage: (id: string, role: 'user' | 'assistant', content: string) =>
    apiFetch<ElaraMessage>(`/api/ai/conversations/${id}/messages`, {
      method: 'POST',
